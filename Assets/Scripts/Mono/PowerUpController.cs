...
public class PowerUpController : MonoBehaviour
{
	...
	void Update ()
    {
        ...
        //loop through the powerups list
        foreach (var item in powerups )
        {
            ...
            //if time is up, move to temp list to  remove
            if (item.durration <= 0)
            {
                expired.Add(item);
                //length = item.durration;

            }

        }

        //now loop through the temp list to remove each power up in the original list
        ...
        //due to expired list being local it will auto clear each time the update function is called
        //however this is how you clear a list
        expired.Clear();
	}
